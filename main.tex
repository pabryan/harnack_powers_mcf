...
For a convex $H^k$-flow satisfying a Harnack inequality we obtain $C^2$-estimates easily.

\begin{lemma}\label{SpeedEst}
On some open interval $(a,b)$ let $M_t,$ $t\in(a,b),$ be a solution of the curvature flow equation
\begin{equation}\label{eq:CurvFlow}
\dot{x}=-\varphi\nu
\end{equation}
for some positive and strictly monotone curvature function $\varphi$ in a spaceform of nonnegative sectional curvature $K\geq 0.$
Then 
...
\begin{equation}
\dot{\tilde{\varphi}}=\fr{\del\varphi}{\del t}(t,x_t),
\end{equation}
