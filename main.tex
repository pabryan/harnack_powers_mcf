...
We are particularly interested in the case \(\varphi(f) = f\), where \(f\) is strictly monotone, 1-homogeneous function of principal curvatures. Our principal result is Harnack inequalities for flows on the sphere when the speed $\p$ is either convex or $\p=H^{\alpha},~ \a\in (0,1)$. This result extends the inequality obtained in \cite{2015arXiv150802821B, bryanlouie} on the sphere. Along the way, we obtain new and old Harnack inequalities in Euclidean space \cite{MR1296393, MR1100812, MR1316556, MR2813400, MR1480081}.
...
We start this section by calculating the evolution equation of $\chi_2$ and a slight modification of it, $\chi_3$, to obtain Harnack estimates for flow by powers of the mean curvature. We will then focused on the evolution equation of $\chi_1$ which will enable us to deduce (weak) Harnack estimates for all 1-homogeneous convex speeds.
\begin{lemma}
\label{thm:Evchi}
Let $\delta \neq 0.$ The quantity
\[
...
To obtain Harnack estimates for all powers $0<\alpha<1$ of the mean curvature, we will have to handle the middle term in \cref{RSphere1}; this term does not always have the favorable positive sign. To do so, it is useful to add an auxiliary function of the speed. Using \cref{RSphere} and  \cref{thm:Evchi}, it is straightforward to check the following evolution equation:
...
in order to get a very strong version of the Harnack estimate, compare Section \ref{Harnack}, Theorem \ref{thm: main 1}. This will turn out to have the drawback that the class of curvature function fulfilling this estimate is very small. In order to cover a larger class of curvature functions, we consider
\eq{\chi_1=\chi_2+tK\p\Phi.}
...
Suppose $f\in C^{\8}(\G_+)$ is a strictly monotone, 1-homogeneous and convex curvature function. If $\p=f$,then for contracting flows
...
In the derivation of the Harnack inequalities we have always assumed to deal with strictly convex hypersurfaces. In this section we want to justify this assumption as reasonable. In particular we want to show that strict convexity is indeed preserved by a variety of flow speeds.
...
\begin{proposition}
Let $f\in C^{\8}(\G_{+})$ be a strictly monotone, 1-homogeneous and convex curvature function and let $0<\a\leq 1$ and in case $\a<1$ assume $f=H$. Let $M_{0}$ be a closed and strictly convex initial hypersurface for the curvature flow equation
\eq{\dot{x}=-f^{\a}\nu.}
Then all flow hypersurfaces are also strictly convex.
\end{proposition}

\pf{
Let us first treat the case $f=H$ and $\a<1.$ The inverse curvature function of $H$ is the harmonic mean curvature
...
where the harmonic mean curvature $\~H$ is now evaluated at $\~\k_i=\k_i^{-1},$ compare \cite{Gerhardt:/2015} for the derivation of the dual flow. As a curvature function, the harmonic mean curvature is 1-homogeneous, strictly monotone, concave and vanishes on the boundary of $\G_{+}.$ For the concavity compare \cite[Lemma~2.2.12, Lemma~2.2.14]{Gerhardt:/2006} for example. For flows of the kind \eqref{DualFlow} uniform curvature estimates were deduced in \cite[Lemma~4.7]{MakowskiScheuer:/2013}, implying that the $\~\k_{i}$ are bounded. This means that up to time $T$ uniform convexity is preserved for the original flow, which contradicts the definition of $T,$ if $T$ is not the collapsing time anyway.

Now suppose that $\a=1.$ We use Hamilton's maximum principle for tensors to deduce that the tensor
\eq{S_{ij}=h_{ij}-\e g_{ij}}
remains nonnegative, if $\e$ is chosen so that this is the case initially. We use the evolution equation \eqref{eq:delt_metric} and \eqref{eq:delt_sff_box} to deduce
\eq{\del_tS_{ij}-\Box S_{ij}&=f^{kl}(h^2)_{kl}h_{ij}-2f(h^2)_{ij}+2fg_{ij}-f^{kl}g_{kl}h_{ij}\\
					&\hp{=}+2\e fh_{ij}+f^{kl,rs}\nabla_{i}h_{kl}\nabla_{j}h_{rs}\\
                    &=:N_{ij}}
At a first unit length null eigenvector $\eta$ of $S_{ij}$ we have
\eq{N_{ij}\eta^i\eta^j\geq -2\e^2f+2f-\e f^{kl}g_{kl}+2\e^2 f,}
where we have used the convexity of $f.$
For convex and $1$-homogeneous curvature functions there holds
\eq{f^{kl}g_{kl}\leq n,}
