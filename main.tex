...
&\quad + (\varphi - 2\varphi^{ij}h_{ij} + \varphi'f)|\nabla\varphi|^{2} & (B') \\
...
&\quad \left. + (\varphi'f + \varphi)b^{ir}b^{j}_{r}\nabla_i \varphi\nabla_j\varphi - 2 b^{j}_{k}\varphi^{kl}\nabla_l\varphi\nabla_j\varphi - 2 b^{ij}\varphi^{kl,rs}\nabla_i h_{rs}\nabla_j\varphi g_{kl}\varphi\right] \\
&= KR \\
...
\end{align*}
where in the last line we use that \(b^{il}(h^2)_{kl} = h^i_k\) and relabel some indices to match terms.
