...
\[
\begin{split}
(\partial_{t} - \Box)\theta &= (\varphi^{rs}(h^2)_{rs} - K\varphi^{kl}g_{kl})\theta - (2b^{lq}\varphi^{kp} + \varphi^{kl,pq})b^{ir}b^{js}\nabla_i \varphi\nabla_j\varphi \nabla_r h_{kl} \nabla_s h_{pq} \\
...
\begin{split}
(\partial_{t}b^{ij} - \Box b^{ij})\nabla_i \varphi\nabla_j\varphi &= \nabla_i \varphi \nabla_j \varphi\left[-\varphi^{rs} (h^2)_{rs} b^{ij} + (\varphi'f + \varphi) g^{ij} \right. \\
& \quad - \left(2b^{lq}\varphi^{kp} + \varphi^{kl,pq}\right) b^{ir}b^{js} \nabla_r h_{kl} \nabla_s h_{pq} \\
& \quad - \left. K \{(\varphi + \varphi'f) b^{ir}b^{j}_{r} + \varphi^{kl}g_{kl}b^{ij}\}\right] \\
&= - \left(K\varphi^{kl}g_{kl} + \varphi^{rs}(h)^2_{rs}\right) \theta - (2b^{lq}\varphi^{kp} + \varphi^{kl,pq}) b^{ir}b^{js}\nabla_i\varphi\nabla_j\varphi\nabla_rh_{kl}\nabla_s h_{pq} \\
&\quad + (\varphi'f + \varphi)|\nabla\varphi|^{2} \\
...
\end{split}
\end{equation}
The first term of \cref{eq:delt_theta2} may be calculated empoying the evolution equation for \(\varphi\), \cref{lem:evolution}, \cref{eq:delt_speed}:
\begin{equation}
